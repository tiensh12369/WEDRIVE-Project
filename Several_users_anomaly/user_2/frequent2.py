...
test_evaluation_file = os.getcwd()+'\\evaluation_2.csv'
...
if os.path.isfile(test_evaluation_file):
    ...
    evaluation_df = pd.DataFrame(columns=['pair', 'F2', 'S1', 'S2', 'lF', 'PF'])
    ...
    if filename.endswith('.csv'):
        ...
        for index, row in df.iterrows():
            ...
            if label == RecurringLabels:
                ...
                previous_label = label
                current_data = pd.DataFrame({'pair': [pair], 'F2': [Anomaly_type], 'PF': [frequency]})
