...
for key, value in sorted_geo_trace_pts.items():
    if value>=2:
        frequemt_pattern[key]=value #정상 패턴
    else:
        normal_pattern[key]=value # 비교 패턴

# 유사도 비교
similarity={}
cell_list = []
 
for key1, value1 in normal_pattern.items():
    similarity[key1]={}
    list1 = list(key1)
   
    for key2, value2 in frequemt_pattern.items():
        if key1 == key2:
            continue
        ...
        if jaro_similarity(list1, list2)>0.80:
            ...
            # similarity[key1][key2]=jaro_similarity(list1, list2)
...
if os.path.isfile(test_evaluation_file):
    ...
    evaluation_df = pd.DataFrame(columns=['label', 'F1', 'S1', 'S2'])
    ...
    if filename.endswith('.csv'):
        ...
        for index, row in df.iterrows():
            ...
            if current_label != label:
                ...
                if pd.read_csv(test_evaluation_file)['label'].empty:
                    current_data = pd.DataFrame({'label': [label], 'S1': [Anomaly_type]})
                else:
                    current_data = pd.DataFrame({'S1': [Anomaly_type]})
